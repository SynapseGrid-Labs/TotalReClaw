...
function expandConfiguredPath(value: string, home: string): string {
  if (value === "~") {
    return home;
  }
  if (value.startsWith("~/")) {
...
export function resolveConfig(raw: unknown, pluginRoot: string): ResolvedConfig {
  ...
  const dbPath = expandConfiguredPath(readString(config.dbPath, path.join(root, "totalreclaw.db")), home);
  const storePath = expandConfiguredPath(readString(config.storePath, path.join(root, "lessons.jsonl")), home);
  const draftPath = expandConfiguredPath(readString(config.draftPath, path.join(root, "review")), home);
  const sessionStatePath = expandConfiguredPath(
    readString(config.sessionStatePath, path.join(root, "state", "sessions")),
    home,
  );
